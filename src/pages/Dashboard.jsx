...
import {
  collection, addDoc, onSnapshot, doc, updateDoc, query, where
  ...
  return (
    <div style={{
      ...
      <nav style={{background: theme==="dark"?"#151d38":"#fff",borderBottom:"2px solid #c2e6ff",boxShadow:"0 2px 8px #1e488210",padding:"13px 0"}}>
        <div style={{maxWidth:500,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{fontWeight:900,fontSize:"1.34em",letterSpacing:".7px",color:accent,display:"flex",alignItems:"center"}}>
            <img src="/logo.png" alt="" style={{height:34,borderRadius:"50%",marginRight:10,background:"#fff"}}/>
            LucixFire Panel
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <span style={{display:"inline-block",border:"2.5px solid #00e3a3",borderRadius:"50%"}}>
              <img src="/logo.png" alt="" style={{height:32,width:32,borderRadius:"50%",background:"#fff"}}/>
            </span>
            <div style={{position:"relative"}}>
              <button onClick={()=>setShowMenu(m=>!m)} style={{
                ...
                outline:showMenu?"2px solid #1ec8fd":"none"}}><FaEllipsisV/></button>
              {showMenu && (
                <div style={{
      ...
      <div style={{
        ...
        <StatCard theme={theme} icon={<FaUser/>} label="Username" value={user?.displayName || user?.email}/>
      ...
      {showTrackOrder && <TrackOrderModal info={orderTrackInfo} orderId={orderTrackId} setId={setOrderTrackId} onTrack={handleTrackOrder} onClose={()=>setShowTrackOrder(false)}/> }
    </div>
  );
}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          style={{
            ...
            color: "#131d33", fontWeight: 900, fontSize: "1.12em", padding: "13px 0", border: "none", marginBottom: 8
          }}
          type="submit"
        >Submit</button>
  ...
  return (
    <Modal title="Profile" onClose={onClose}>
      <div style={{textAlign:"center",padding:14}}>
        ...
        <div><b style={{fontWeight:900}}>{user?.displayName || user?.email}</b></div>
...
function TrackOrderModal({info,orderId,setId,onTrack,onClose}) {
  return (
    <Modal title="Track Order" onClose={onClose}>
      <form onSubmit={onTrack}>
        ...
        <div style={{marginTop:13,color:info&&info.startsWith("Order not found!")?"#ca4261":"#2bd77e",whiteSpace:"pre-wrap",fontWeight:700}}>{info||""}</div>
      </form>
    </Modal>
  );
}
